...
export default async function ClientProgressPage({
  ...
  const { data: client } = await supabase
    .from("clients")
    .select("id, full_name, plan_type, city")
    .eq("id", id)
    .single();
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-ink p-8">
        <p className="text-[10px] tracking-widest uppercase text-warm/40 mb-2 font-body">
          Progress Review
        </p>
        ...
        <p className="text-cream/60 text-sm font-body">
          {client.city} · {client.plan_type?.replace("_", " ").replace("week", "/ week")}
        </p>
      ...
      <TrainerProgressActions clientId={id} />
              ...
              <div key={photo.id} className="bg-white border border-stone">
                ...
                <div className="flex items-center justify-between gap-4 p-5 border-b border-stone">
                  <div>
                    <p className="font-display text-lg font-light text-ink">
                      {new Date(photo.recorded_at).toLocaleDateString("en-US", {
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-muted font-body">
                    {photo.weight_lbs && (
                      <span><strong className="text-ink">{photo.weight_lbs} lbs</strong> weight</span>
                    )}
                    {photo.energy_level && (
                      <span><strong className="text-ink">{photo.energy_level}/5</strong> energy</span>
                    )}
                    {photo.sleep_quality && (
                      <span><strong className="text-ink">{photo.sleep_quality}/5</strong> sleep</span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-px bg-stone">
                  {[
                    ...
                    <div key={key} className="bg-white aspect-[3/4] relative overflow-hidden">
                      ...
                      <span className="absolute bottom-2 left-2 bg-black/50 text-white text-[9px] tracking-widest uppercase font-body px-1.5 py-0.5">
                        {label}
                      </span>
                    </div>
                  ))}
                ...
                {(photo.wins || photo.challenges) && (
                  <div className="px-5 py-4 grid grid-cols-2 gap-4 border-t border-stone">
                    {photo.wins && (
                      <div>
                        <p className="text-[10px] tracking-widest uppercase text-muted mb-1 font-body">
                          Client Wins
                        </p>
                ...
                <div className="px-5 pb-5 border-t border-stone pt-4">
                  {photo.trainer_comment ? (
                    <div>
                      ...
                      <p className="text-sm text-ink font-body leading-relaxed italic bg-ink/5 border-l-2 border-ink px-4 py-3">
                        {photo.trainer_comment}
                      </p>
                    </div>
                  ) : (
      ...
      {/* Monthly assessments */}
      {progress && progress.filter((p: any) => p.is_monthly_review).length > 0 && (
        <div>
          <p className="text-[10px] tracking-widest uppercase text-muted mb-4 font-body">
            Monthly Assessments
          </p>
